...
package org.mmbase.applications.te.minixml;
 ...
 * XML code.
 * <h1>this software is based on nanoxml</h1>
 * <P>
 * <UL><LI>The parser is non-validating.</LI>
 *     <LI>The DTD is fully ignored, including <CODE>&lt;!ENTITY...&gt;</CODE>.</LI>
 *     <LI>Kees jongenburger: added support for mixed content</LI>
 * </UL>
 * <P>
  * @see minixml.XMLParseException
 * @author original author Marc De Scheemaecker
...
public class XMLElement implements Serializable {
     ...
     * </DL>
     *
     * @see minixml.XMLElement#XMLElement(java.util.Properties)
     * @see minixml.XMLElement#XMLElement(boolean)
     * @see minixml.XMLElement#XMLElement(java.util.Properties,boolean)
     */
    public XMLElement() {
     ...
     * </DL>
     *
     * @see minixml.XMLElement#XMLElement()
     * @see minixml.XMLElement#XMLElement(boolean)
     * @see minixml.XMLElement#XMLElement(java.util.Properties,boolean)
     */
     ...
     * @exception minixml.XMLParseException
     ...
     *
     * @see minixml.XMLElement#decodeString
     *
     * @exception minixml.XMLParseException
     *    if an error occured while parsing the array
     */
    private void processContents(char[] input, int contentOffset, int contentSize, int contentLineNr) throws XMLParseException {
        int[] lineNr = new int[1];
     ...
     *         input[offset] in { '/', '>' }
     *
     * @see minixml.XMLElement#scanOneAttribute
     *
     * @exception minixml.XMLParseException
     *    if an error occured while parsing the array
     */
     ...
     *
     * @see minixml.XMLElement#parseCharArray
     *
     * @exception minixml.XMLParseException
     *    if an error occured while parsing the array
     */
            ...
            if ((input[offset] != '<') || ((input[offset + 1] == '!') && (input[offset + 2] == '['))) {
                ...
                if (count == end) {
                    if (children.size() > 0) {
                        ...
                        child.setContent(new String(input, start, count - start).trim());
								child.parent = this;
                        this.children.addElement(child);
                    }
                    return;
                    ...
                    XMLElement child = this.createAnotherElement();
                    child.setContent(new String(input, start, count - start));
						  child.parent = this;
                    this.children.addElement(child);
                    if (input[count + 1] == '!' && input[count + 2] == '[') {
                        ...
                        //<![CDATA[  ]]>//
                        String section = new String(input, count, end - count);
                        int endIndex = section.indexOf("]]>");

                        XMLElement cdata = this.createAnotherElement();
                        cdata.isCDATA = true;
                        String cdataContent = new String(input, count, endIndex + 3);
                        cdata.setContent(decodeString(cdataContent, contentLineNr));
								cdata.parent = this;
                        this.children.addElement(cdata);
                        this.scanChildren(input, count + endIndex + 3, (end - count) - (endIndex + 3), contentLineNr);
                    } else {
                        this.scanChildren(input, count, end - count, contentLineNr);
                    }
     ...
     *    if an error occured while parsing the array
     */
    private int scanOneAttribute(char[] input, int offset, int end, int[] lineNr) throws XMLParseException {
        String key, value;
     ...
     *
     * @see minixml.XMLElement#scanIdentifier
     *
     * @exception minixml.XMLParseException
     *    if an error occured while parsing the array
     */
    private String scanString(char[] input, int offset, int end, int[] lineNr) throws XMLParseException {
     ...
     *    if an error occured while parsing the array
     */
    private int scanTagName(char[] input, int offset, int end, int[] lineNr) throws XMLParseException {
    ...
    public void write(Writer writer, int indent) {
        ...
        if (this.tagName == null || this.isTextNode) {
